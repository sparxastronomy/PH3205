...
% for a particle trapped in a unequal double well
%----------------------------------------------------------------
clc
clear all
% Potential due to two unequal square wells
...
a = L/30;                % width
b = L/50;                % separation
U = -200*(heaviside(x+0.5*a+ 0.5*b)-heaviside(x+0.5*a) ...
    +heaviside(x-0.5*a)-heaviside(x-0.5*a-2*b));
...
% Rescale eigenvectors so that they are always positive at the center of the right well.
for c = 1:nmodes
...
vw1 = VideoWriter('UnequalDoubleWell.avi'); % Prepare the new file.
